...
| Tool | Input | Output |
| --- | --- | --- |
| `search_articles` | `query`, `since`, `sourceId`, `unreadOnly`, `limit` | id, title, url, publishedAt, summary, score |
| `get_article` | `id`, `format`: `markdown` \| `text` \| `summary` | One article including body |
